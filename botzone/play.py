...
class OthelloAI:
    ...
    def load_endgame(self):
        ...
        try:
            ...
            so_path = os.path.join(base_dir, "data", "ending.so")
    ...
    def run(self):
        ...
        # 统一 debug 字段
        dbg_parts = [f"empties={empties}"]
        if empties <= self.ENDGAME_MOVE_THRESHOLD:
            if self.endgame_fn:
                if self.last_endgame_value is not None:
                    if self.last_endgame_time_ms is not None:
                        dbg_parts.append(f"solver=OK val={self.last_endgame_value} t={self.last_endgame_time_ms:.2f}ms")
                    else:
                        dbg_parts.append(f"solver=OK val={self.last_endgame_value}")
                else:
                    dbg_parts.append("solver=OK no_call")
            else:
                if self.endgame_error:
                    dbg_parts.append(f"solver=FAIL {self.endgame_error}")
                else:
                    dbg_parts.append("solver=MISS")
        else:
            dbg_parts.append("solver=SKIP")
        out["debug"] = ';'.join(dbg_parts)
