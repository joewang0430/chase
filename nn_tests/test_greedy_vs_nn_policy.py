...
class NNPolicyBaseline:
    ...
    def _load_weights(self, path: str):
        ...
        # Support checkpoints that may wrap state dict
    ...
    def choose_move(self, my: int, opp: int) -> int:
        ...
        if legal == 0:
            return PASS_INDEX
        # Build input planes with legal mask included
        x = build_input_planes(my, opp, legal)
        with torch.no_grad():
            logits, _ = self.net(x)
            logits = logits.squeeze(0)  # (65,)
        # Mask illegal moves by setting -inf
        # Collect legal move indices 0..63
        legal_indices = []
        m = legal
        while m:
            lsb = m & -m
        ...
        # Only positions in legal_indices are allowed; pass (64) is NOT allowed when legal exists
        for idx in legal_indices:
            mask[idx] = 0.0
        masked_logits = logits + mask
        move = int(torch.argmax(masked_logits).item())
        # Safety: ensure selected move is legal
        if move == PASS_INDEX or (((legal >> move) & 1) == 0):
            move = legal_indices[0]
        return move
...
def play_one_game(nn_is_black: bool, policy: NNPolicyBaseline, rng: random.Random, dual_counter: dict):
    ai = OthelloAI()
    ...
    while True:
        ...
        # Opening randomization: both sides play random in the first 6 plies (total pieces <= 10)
        total_pieces = popcount((black | white) & BOARD_MASK)
        if total_pieces <= 10:
            # random opening move regardless of side
            mv = rnd.choose_move(my, opp)
            if mv != PASS_INDEX:
        ...
        # Midgame move
        if side_black_to_move:
            mv = policy.choose_move(my, opp) if nn_is_black else greedy_choose(my, opp)
        else:
            mv = greedy_choose(my, opp) if nn_is_black else policy.choose_move(my, opp)
...
def main():
    ...
    ap.add_argument('--model', type=str, default=os.path.join('models', 'value_only_best.pt'))
    args = ap.parse_args()
    ...
    policy = NNPolicyBaseline(args.model)
    ...
    for _ in range(games // 2):
        fb, fw = play_one_game(nn_is_black=True, policy=policy, rng=random, dual_counter=dual)
        g_black.add(fb, fw, side_is_black=True)
    # NN as White
    for _ in range(games // 2):
        fb, fw = play_one_game(nn_is_black=False, policy=policy, rng=random, dual_counter=dual)
    ...
    print("=== nn-policy vs greedy ({} games) ===".format(g_all.n))
