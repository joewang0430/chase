# PYTHONPATH=. python3 nn_tests/test_policy_guarantee.py --model models/chase.pt --seed 42 --games-per-opponent 200
...
class EndgameLib:
    ...
    def _load(self):
        ...
        # bind solve_endgame(my, opp, int* best_move)
...
def play_one_game_vs_engine(nn_policy: NNPolicy, engine: CEngine, nn_is_black: bool, rng: random.Random):
    ...
    while True:
        ...
        if empties <= 8:
            # Dual-path endgame check: play_to_end vs per-move solve_endgame
            b0, w0, s0 = black, white, side_black_to_move
            ok, fb1, fw1 = END_LIB.finish_with_endgame_myopp(b0, w0, s0)
            if ok:
                fb2, fw2 = endgame_by_per_move_solve(b0, w0, s0)
                # Accumulate globally via a closure or external counter; handled by caller via global dict
                dual_counter = globals().get('_DUAL_COUNTER_ENGINE', None)
                if dual_counter is not None:
...
def play_one_game_vs_greedy(nn_policy: NNPolicy, nn_is_black: bool, rng: random.Random):
    ...
    while True:
        ...
        if empties <= 8:
            ...
            if ok:
                fb2, fw2 = endgame_by_per_move_solve(b0, w0, s0)
                dual_counter = globals().get('_DUAL_COUNTER_GREEDY', None)
                if dual_counter is not None:
                    dual_counter['samples'] = dual_counter.get('samples', 0) + 1
                    if (fb1 & BOARD_MASK) != (fb2 & BOARD_MASK) or (fw1 & BOARD_MASK) != (fw2 & BOARD_MASK):
                        dual_counter['mismatch'] = dual_counter.get('mismatch', 0) + 1
...
def main():
    ...
    ap.add_argument('--model', type=str, default=os.path.join('models', 'chase.pt'), help='NN 权重路径')
    ...
    for name, eng in engines:
        ...
        st_black = Stats()
        st_white = Stats()
        # dual-endgame consistency counter per engine
        dual_engine = {'samples': 0, 'mismatch': 0}
        globals()['_DUAL_COUNTER_ENGINE'] = dual_engine
        ...
        print(f"[dual-endgame consistency] samples={dual_engine.get('samples',0)}, mismatches={dual_engine.get('mismatch',0)}")
    ...
    dual_greedy = {'samples': 0, 'mismatch': 0}
    globals()['_DUAL_COUNTER_GREEDY'] = dual_greedy
    ...
    print(f"[dual-endgame consistency] samples={dual_greedy.get('samples',0)}, mismatches={dual_greedy.get('mismatch',0)}")
